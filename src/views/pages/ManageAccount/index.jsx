...
function ManageAccount() {
  ...
  const onClickChangeDeleteStatus = async (id, isDeleted) => {
    try {
      if (isDeleted === 0) {
        const response = await AccountAPI.activateAccount(id);
        if (response.data.is_success) {
          ...
          toast.success("Tài khoản đã được kích hoạt!", { autoClose: 2000 });
        }
      } else {
    ...
    if (selectedAccount) {
      ...
      try {
        ...
        if (response.data.is_success) {
          const updatedRecords = filteredRecords.map((record) =>
            record.id === id
              ? { ...record, status: isDeleted === 1 ? "INACTIVE" : "ACTIVE" }
              : record
          );
          setFilteredRecords(updatedRecords);
          setAccountRecords(updatedRecords);
