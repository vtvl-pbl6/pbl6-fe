...
const NotificationModal = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      if (visible) {
        try {
          const receivedResponse = await NotificationAPI.getNotifications(1);
          const sentResponse = await NotificationAPI.getCreatedNotifications(1);
          console.log(receivedResponse.data.data);
          console.log(sentResponse.data.data);

          if (receivedResponse.data.is_success) {
            const receivedNotifications = receivedResponse.data.data.map(
              (noti) => ({
                id: noti.object_id,
          ...
          if (sentResponse.data.is_success) {
            const sentNotifications = sentResponse.data.data.map((noti) => ({
              id: noti.id,
          ...
          renderItem={(noti) => (
            <List.Item className="list-item">
              <List.Item.Meta
                ...
                description={<span className="time">{noti.time}</span>}
              />
